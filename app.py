...
try:
    ...
    st.error("⚠️ เชื่อมต่อขัดข้อง")
    st.stop()

# กู้คืน Session จาก URL (กันเด้ง)
...
# --- 4. CSS เฉพาะส่วนภารกิจ (ฟ้า-จิ๋ว-ชิด) ---
st.markdown("""
    <style>
        /* 🔵 1. ปุ่มภารกิจกรอบฟ้าจิ๋ว */
        .thin-btn-blue div.stButton > button {
            background-color: transparent !important;
            color: #1877f2 !important;
            border: 1px solid #1877f2 !important;
            padding: 0px 5px !important;
            height: 22px !important;    /* เตี้ยสุดๆ */
            min-height: unset !important;
            font-size: 11px !important;  /* เล็กสุดๆ */
        ...
        /* 🛑 3. บีบช่องว่างระหว่างบรรทัดให้ชิดกัน (สำหรับ 60 ภารกิจ) */
        [data-testid="column"] {
            padding: 0px !important;
            margin-bottom: -23px !important; /* ดึงบรรทัดล่างขึ้นมาเบียด */
        }
        
        /* ลบระยะห่างส่วนเกินของ Streamlit */
        .stElementContainer { margin-bottom: -12px !important; }
        
        hr { margin: 5px 0px !important; opacity: 0.2; }
    </style>
...
# 🎮 หน้ากิจกรรม (Player)
    ...
    if st.session_state.selected_mission is None:
        ...
        if f and st.button("ยืนยัน", use_container_width=True):
...
# หน้าอื่น (Signup/Forgot) ใส่โครงไว้กัน Error
elif st.session_state.page == 'signup':
    if st.button("⬅️ กลับ"): go_to('login')
