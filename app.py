...
# --- 3. CSS ฉบับ "ลอกคราบปุ่มให้เป็นลิงก์" (คงความสวยงามเดิมที่พี่ชอบ) ---
st.markdown("""
    <style>
        ...
        /* 🔗 🛑 หัวใจหลัก: เปลี่ยนปุ่มให้กลายเป็น HTML Link ตัวหนังสือเพียวๆ */
        /* ลบพื้นหลัง, ลบขอบ, ลบเงา, ตั้งค่าให้เหมือนตัวหนังสือ */
        .magic-link button {
            background: none !important;
            border: none !important;
            padding: 0 !important;
            color: #1877f2 !important;
            text-decoration: underline !important; /* มีขีดเส้นใต้แบบลิงก์ */
        ...
        .status-done { color: #42b72a; font-weight: bold; font-size: 14px; margin-left: 5px; }
        .status-pending { color: #888; font-size: 14px; margin-left: 5px; }
    </style>
...
# 🎮 หน้ากิจกรรม (Player)
    ...
    if st.session_state.selected_mission is None:
        ...
        for m in missions:
            ...
            status_text = '<span class="status-done">(✅ ส่งแล้ว)</span>' if is_done else '<span class="status-pending">(⭕ รอดำเนินการ)</span>'
            
            # ✨ ลิงก์รายชื่อด่าน (ตัวหนังสือลิงก์ ไม่เด้งออก)
            st.markdown('<div class="magic-link" style="margin-bottom: 10px;">', unsafe_allow_html=True)
    ...
    # --- กรณีหน้าทำกิจกรรม ---
        ...
        if sub_check:
            ...
            if f and st.button("ยืนยันส่งงาน", type="secondary", use_container_width=True):
                # ... (โค้ดอัปโหลด Drive/Supabase คงเดิม) ...
                st.success("🎉 สำเร็จ!"); time.sleep(1); st.session_state.selected_mission = None; st.rerun()
        
        # ✨ ลิงก์ย้อนกลับ (ตัวหนังสือลิงก์)
        st.markdown('<div class="magic-link" style="margin-top:20px;">', unsafe_allow_html=True)
...
# (หน้า Signup และ Forgot พี่ก๊อปอันเดิมมาใส่ได้เลยครับ ผมคง Logic เดิมไว้ให้ทั้งหมด)
