...
# =========================================================
# 🎮 หน้า BONUS GAME: ระบบวิ่งเก็บหมวก + ตู้สุ่มกาชา (ฉบับแก้ไข)
# =========================================================
elif st.session_state.page == 'bonus_game':
    u = st.session_state.user
    st.markdown("<h2 style='text-align: center; color:#1877f2; margin-bottom:0;'>🏃‍♂️ Traffic Runner & Gacha</h2>", unsafe_allow_html=True)

    # --- 1. เช็กสิทธิ์การเข้าเล่น (ส่งงาน 1 ครั้ง = สุ่มได้ตามโควต้า) ---
    try:
        # นับจำนวนงานที่ส่งทั้งหมดของเด็กคนนี้
        ...
        # ดึงจำนวนครั้งที่สุ่มไปแล้วจากตาราง users
        played_count = u.get('game_played_count', 0)
        available_play = mission_count - played_count
    except:
        mission_count, played_count, available_play = 0, 0, 0

    # แสดงแถบสถานะสิทธิ์ด้านบน
    st.info(f"🎟️ สิทธิ์ของคุณ: ส่งงานแล้ว {mission_count} ครั้ง | ใช้ไปแล้ว {played_count} ครั้ง | **คงเหลือ {max(0, available_play)} สิทธิ์**")

    # --- 2. ตัวมินิเกม (แสดงให้เห็นตลอดเพื่อความเร้าใจ) ---
    import streamlit.components.v1 as components
    # (ใช้ game_html ที่พี่มีอยู่ด้านบนในไฟล์)
    components.html(game_html, height=380) 

    st.markdown("---")

    # --- 3. ระบบสุ่มรางวัล (Gacha Section) ---
    st.subheader("🎁 ตู้สุ่มรางวัล EXP")
    st.markdown(f"""
    <div style='background: #f0f2f6; padding: 15px; border-radius: 10px; border-left: 5px solid #1877f2;'>
        📌 <b>กติกา:</b> ทุกๆ <b>300 แต้ม</b> ในเกม รับสิทธิ์สุ่ม 1 ครั้ง!<br>
        💎 <b>โอกาสสุ่ม:</b> 100 EXP(3%) | 50 EXP(7%) | 20 EXP(15%) | 10 EXP(35%) | 5 EXP(40%)
    </div>
    """, unsafe_allow_html=True)
    ...
    # ส่วนระบุคะแนน
    game_score = st.number_input("กรอกคะแนนที่ทำได้จากเกม:", min_value=0, step=100, key="score_input")
    spins_earned = int(game_score // 300) # ทุก 300 แต้ม = 1 ครั้ง
    
    if spins_earned > 0:
        st.success(f"✨ ยอดเยี่ยม! คะแนน {game_score} แต้ม ทำให้คุณได้รับสิทธิ์สุ่ม **{spins_earned}** ครั้ง")
        
        # เช็กว่ามีตั๋ว (งานที่ส่ง) เหลือไหม
        if available_play > 0:
            if st.button(f"🎰 กดสุ่มรางวัล {spins_earned} ครั้ง (ใช้ 1 สิทธิ์เข้าเล่น)", type="primary", use_container_width=True):
                # --- 🎯 Logic การสุ่มแบบถ่วงน้ำหนัก (Weighted Random) ---
                exp_options = [5, 10, 20, 50, 100]
                weights = [40, 35, 15, 7, 3] # เปอร์เซ็นต์ตามที่พี่กำหนด
                
                results = []
                total_win = 0
                for _ in range(spins_earned):
                    win = random.choices(exp_options, weights=weights, k=1)[0]
                    results.append(win)
                    total_win += win
                
                # --- บันทึกข้อมูล ---
                try:
                    new_total_exp = (u.get('total_exp', 0)) + total_win
                    new_played_count = played_count + 1
                    
                    supabase.table("users").update({
                        "total_exp": new_total_exp,
                        "game_played_count": new_played_count
                    }).eq("username", u['username']).execute()
                    
                    # อัปเดต Session
                    st.session_state.user['total_exp'] = new_total_exp
                    st.session_state.user['game_played_count'] = new_played_count
                    
                    st.balloons()
                    st.success(f"🎊 ยินดีด้วย! คุณได้รับ EXP: {' + '.join(map(str, results))} รวม **+{total_win} EXP**")
                    time.sleep(2)
                    st.rerun()
                except Exception as e:
                    st.error(f"บันทึกรางวัลล้มเหลว: {e}")
        else:
            st.warning("⚠️ คุณมีคะแนนพอ แต่ **'สิทธิ์เข้าเล่น (ตั๋ว)'** หมด! กรุณาส่งภารกิจเพิ่มเพื่อรับสิทธิ์สุ่ม")
    else:
        st.info("🏃‍♂️ วิ่งเก็บแต้มให้ถึง 300 เพื่อเปิดตู้สุ่มรางวัลนะ!")

    # ปุ่มกลับหน้าหลัก
    if st.button("⬅️ กลับไปหน้าหลัก", use_container_width=True, key="back_from_game"):
        st.session_state.page = 'game'
        st.rerun()
# =========================================================
