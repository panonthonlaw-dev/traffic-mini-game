...
# =========================================================
# 🎮 หน้า BONUS GAME: ระบบ Gacha ตามจำนวนงาน
# =========================================================
elif st.session_state.page == 'bonus_game':
    u = st.session_state.user
    
    # --- 1. เช็กสิทธิ์การเล่น (จำนวนงานที่ส่ง vs จำนวนครั้งที่เล่น) ---
    # ดึงจำนวนงานที่ส่งทั้งหมด
    mission_count = supabase.table("submissions").select("id", count="exact").eq("user_username", u['username']).execute().count
    # ดึงจำนวนครั้งที่เคยเล่นไปแล้ว
    played_count = u.get('game_played_count', 0)
    
    available_play = mission_count - played_count
    
    st.markdown(f"<h2 style='text-align: center;'>🎰 ตู้สุ่มรางวัล EXP</h2>", unsafe_allow_html=True)
    st.write(f"📊 ส่งงานแล้ว: **{mission_count}** ครั้ง | ใช้สิทธิ์ไปแล้ว: **{played_count}** ครั้ง")
    
    if available_play <= 0:
        st.warning("⚠️ สิทธิ์การเล่นหมดแล้ว! ต้องส่งภารกิจเพิ่มเพื่อรับสิทธิ์เล่นเกมครั้งต่อไป")
        if st.button("⬅️ กลับไปทำภารกิจ"):
            st.session_state.page = 'game'
            st.rerun()
    else:
        st.success(f"🎟️ คุณมีสิทธิ์เล่นได้อีก **{available_play}** ครั้งในรอบนี้")
        
        # --- 2. ตัวเกม (เรียก HTML เกมเดิมที่พี่มี) ---
        # ... (ส่วนประกอบ components.html(game_html) ของพี่) ...
        
        st.write("---")
        st.subheader("🎁 แลกรางวัลจากคะแนน")
        
        # สมมติว่าพี่ได้คะแนนจาก JS มาใส่ในตัวแปรนี้ (หรือให้เด็กกรอก/ส่งค่ากลับมา)
        game_score = st.number_input("ใส่คะแนนที่ทำได้ในรอบนี้", min_value=0, step=100)
        
        # คำนวณสิทธิ์สุ่ม: ทุกๆ 300 แต้ม = 1 สิทธิ์
        spins_earned = game_score // 300
        
        if spins_earned > 0:
            st.info(f"✨ คะแนน {game_score} แต้ม ทำให้คุณได้รับสิทธิ์สุ่ม **{int(spins_earned)}** ครั้ง!")
            
            if st.button(f"🎰 เริ่มการสุ่ม {int(spins_earned)} ครั้ง", type="primary", use_container_width=True):
                rewards_list = []
                total_reward_exp = 0
                
                # --- 🎯 Logic การสุ่มแบบถ่วงน้ำหนัก ---
                # อัตรา: 5(40%), 10(35%), 20(15%), 50(7%), 100(3%)
                exp_options = [5, 10, 20, 50, 100]
                weights = [40, 35, 15, 7, 3]
                
                for _ in range(int(spins_earned)):
                    win = random.choices(exp_options, weights=weights, k=1)[0]
                    rewards_list.append(win)
                    total_reward_exp += win
                
                # --- 3. อัปเดต Database ---
                try:
                    new_exp = (u.get('total_exp', 0)) + total_reward_exp
                    new_played = played_count + 1 # ใช้ไป 1 สิทธิ์การเข้าเล่น
                    ...
                    # แสดงผล
                    st.balloons()
                    st.success(f"🎊 ยินดีด้วย! คุณสุ่มได้ EXP: {rewards_list}")
                    st.markdown(f"### 🏆 รวมได้รับทั้งหมด: **+{total_reward_exp} EXP**")
                    time.sleep(2)
                    st.rerun()
                except Exception as e:
                    st.error(f"เกิดข้อผิดพลาดในการบันทึกรางวัล: {e}")
        else:
            st.write("🏃‍♂️ วิ่งให้ถึง **300 แต้ม** เพื่อรับสิทธิ์สุ่มรางวัลนะ!")
