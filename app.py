...
# =========================================================
# 🎮 หน้า BONUS GAME: วิ่งเก็บหมวก + ตู้กาชาสุ่ม EXP
# =========================================================
elif st.session_state.page == 'bonus_game':
    u = st.session_state.user
    st.markdown("<h2 style='text-align: center; color:#1877f2;'>🏃‍♂️ Traffic Runner & Gacha</h2>", unsafe_allow_html=True)

    # --- 1. คำนวณสิทธิ์จากฐานข้อมูล ---
    try:
        # นับจำนวนงานที่ส่งทั้งหมด (ถือว่าเป็น "ตั๋ว" เข้าสุ่ม)
        m_count_res = supabase.table("submissions").select("id", count="exact").eq("user_username", u['username']).execute()
        mission_count = m_count_res.count if m_count_res.count else 0
    ...
    # แสดงสถานะตั๋ว
    st.info(f"🎫 **ตั๋วกาชาของคุณ:** ส่งงานแล้ว {mission_count} ครั้ง | ใช้ไปแล้ว {played_count} ครั้ง | **เหลือสิทธิ์สุ่ม {max(0, available_tickets)} ครั้ง**")

    # --- 2. ตัวมินิเกม (แสดงให้เล่นได้ตลอดเวลา) ---
    import streamlit.components.v1 as components
    # (หมายเหตุ: ใช้ตัวแปร game_html เดิมที่พี่มีในไฟล์)
    components.html(game_html, height=400) 

    st.write("---")

    # --- 3. ระบบตู้สุ่มรางวัล (จะโชว์ให้เห็นตลอด แต่กดสุ่มได้เมื่อเงื่อนไขครบ) ---
    st.subheader("🎁 ตู้สุ่มรางวัลนำโชค")
    
    # แสดงตารางโอกาสสุ่ม (Probability) แบบสวยๆ ให้เด็กดู
    with st.expander("🔍 ดูอัตราการออกรางวัล (Gacha Rate)"):
        st.write("อัตราการสุ่มคำนวณตามหลักสถิติ:")
        st.markdown("""
        * **100 EXP** (Legendary) : 3%
        * **50 EXP** (Epic) : 7%
        * **20 EXP** (Rare) : 15%
    ...
    # ช่องกรอกคะแนนที่ได้จากเกม
    score_input = st.number_input("ใส่คะแนนที่ทำได้จากเกมในรอบนี้:", min_value=0, step=100)
    spins_to_get = int(score_input // 300) # ทุกๆ 300 แต้ม = 1 สิทธิ์สุ่ม

    if score_input > 0:
        if spins_to_get > 0:
            st.success(f"✨ ยอดเยี่ยม! คะแนน {score_input} แต้ม แลกสิทธิ์สุ่มได้ **{spins_to_get}** ครั้ง")
            
            # เช็กว่ามีตั๋ว (งานที่ส่ง) เหลือพอไหม
            if available_tickets > 0:
                if st.button(f"🎰 กดสุ่มรางวัล {spins_to_get} ครั้ง", type="primary", use_container_width=True):
                    # --- 🎯 Logic การสุ่มแบบถ่วงน้ำหนัก (Weighted Random) ---
                    # 5(40%), 10(35%), 20(15%), 50(7%), 100(3%)
                    exp_pool = [5, 10, 20, 50, 100]
                    exp_weights = [40, 35, 15, 7, 3]
                    
                    won_list = []
                    total_won_exp = 0
                    
                    for _ in range(spins_to_get):
                        # สุ่มแบบถ่วงน้ำหนักด้วย random.choices
                        win = random.choices(exp_pool, weights=exp_weights, k=1)[0]
                        won_list.append(win)
                        total_won_exp += win
                    
                    # บันทึกลงฐานข้อมูล
                    try:
                        new_total_exp = (u.get('total_exp', 0)) + total_won_exp
                        new_played_count = played_count + 1 # หักตั๋วออก 1 ใบต่อการกดสุ่ม 1 รอบ
                        ...
                        # อัปเดตข้อมูลใน Session ทันที
                        st.session_state.user['total_exp'] = new_total_exp
                        st.session_state.user['game_played_count'] = new_played_count
                        
                        # แสดงผลความเฮง
                        st.balloons()
                        st.success(f"🎊 ยินดีด้วย! คุณได้รับ EXP: {' + '.join(map(str, won_list))} รวมเป็น **+{total_won_exp} EXP**")
                        time.sleep(2)
                        st.rerun()
                    except Exception as e:
                        st.error(f"ระบบขัดข้อง: {e}")
            else:
                st.warning("⚠️ แต้มถึง...แต่ **'ตั๋ว'** หมด! กรุณาส่งภารกิจเพื่อรับตั๋วเพิ่ม")
        else:
            st.info("🏃‍♂️ อีกนิดเดียว! วิ่งให้ถึง **300 แต้ม** เพื่อเปิดตู้สุ่มรางวัล")
