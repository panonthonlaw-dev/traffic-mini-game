...
# 🟢 หน้าสมัครสมาชิก (เพิ่มรหัสนักเรียน + Validation)
    ...
    with col:
        with st.form("signup_form"):
            ...
            if submit:
                ...
                # ตรวจสอบรหัสนักเรียน (ตัวเลขเท่านั้น)
                if not sid.isdigit():
                    errors.append("❌ รหัสนักเรียนต้องเป็นตัวเลขเท่านั้น")
                
                # ตรวจสอบชื่อไทย
                if not re.match(r'^[ก-ฮะ-์\s]+$', fullname):
                    errors.append("❌ ชื่อ-นามสกุล ต้องเป็นภาษาไทยเท่านั้น")
                
                # ตรวจสอบชื่อผู้ใช้ (6-12 ตัว)
                if not re.match(r'^[a-zA-Z0-9]{6,12}$', username):
                    errors.append("❌ ชื่อผู้ใช้ต้องเป็นภาษาอังกฤษหรือตัวเลข 6-12 ตัวอักษร")
                
                # ตรวจสอบเบอร์โทร
                if not re.match(r'^0[689][0-9]{8}$', phone):
                    errors.append("❌ เบอร์โทรต้องมี 10 หลัก และขึ้นต้นด้วย 06, 08 หรือ 09")
                
                # ตรวจสอบรหัสผ่าน (6-12 ตัว)
                if not re.match(r'^[a-zA-Z0-9]{6,12}$', password):
                    errors.append("❌ รหัสผ่านต้องเป็นภาษาอังกฤษหรือตัวเลข 6-12 ตัวอักษร")
                ...
                if errors:
                    ...
                    try:
                        # บันทึกข้อมูลลง Supabase (อย่าลืมเพิ่มคอลัมน์ student_id ในตาราง users นะครับ)
                        ...
                        st.error("❌ ชื่อผู้ใช้นี้อาจมีคนใช้แล้ว")
...
# 🎮 หน้าหลัก/ภารกิจ
    ...
    with col:
        st.markdown('<div class="mission-card"><b>รหัสนักเรียน:</b> '+str(u.get('student_id', 'ไม่มีข้อมูล'))+'</div>', unsafe_allow_html=True)
